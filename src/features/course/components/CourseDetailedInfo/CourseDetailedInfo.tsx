...
import {
  ...
  StudentStatusTexts,
  statusTexts,
} from '@/utils/constants/statusTexts';
import {
  getStatusColor,
  getStudentMarkColor,
    ...
    <div>
        ...
        <CardContent className='space-y-2'>
          <div className='flex justify-between flex-auto border-2 p-3 rounded-md items-center'>
            <div className='flex flex-col'>
              ...
              <span className={cn(getStatusColor(course.status))}>
      ...
      <Tabs defaultValue='requirements' className='m-2 border-2 p-3 rounded-md'>
        <TabsList className='w-full'>
          <TabsTrigger value='requirements' className='flex-auto'>
            Требования
          </TabsTrigger>
          <TabsTrigger value='annotations' className='flex-auto'>
            Аннотации
          </TabsTrigger>
          ...
          <div>
            ...
            {course.notifications.map((notification, index) => (
              <div
                key={index}
                className={cn(
                  'flex flex-col border-2 p-4 rounded-md',
                  notification.isImportant && 'bg-destructive',
                )}
              >
      ...
      <Tabs defaultValue='teachers' className='m-2 border-2 p-3 rounded-md'>
        <TabsList className='w-full'>
          <TabsTrigger value='teachers' className='flex-auto'>
            Преподаватели
          </TabsTrigger>
          <TabsTrigger value='students' className='flex-auto'>
            Студенты
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value='students' className='space-y-2'>
          {course.students.map((student) => (
            <div
              key={student.id}
              className='flex border-2 p-4 rounded-md items-center overflow-auto gap-6'
            >
              <div className='flex flex-col flex-auto'>
                ...
                <span>
                  Статус -{' '}
                  <span className={cn(getStudentStatusColor(student.status))}>
                    {StudentStatusTexts[student.status]}
                  </span>
                </span>
              </div>
              ...
              {student.status !== 'Accepted' && student.status !== 'Declined' && (
                <div className='flex gap-2 my-2'>
                  <Button
                    onClick={() => handleChangeUserStatus(student.id, 'Accepted')}
                    loading={isEditStatusLoading}
                  >
                    Принять
                  </Button>
                  <Button
                    onClick={() => handleChangeUserStatus(student.id, 'Declined')}
                    loading={isEditStatusLoading}
                  >
                    Отклонить
                  </Button>
